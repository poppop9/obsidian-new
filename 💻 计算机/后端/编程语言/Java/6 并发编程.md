...
## 💛 Future
>[!quote] Future
>Future 是异步执行过程和结果的载体，是一个泛型接口。我们可以将某个任务交给 Future 来异步处理，我们可以去处理其他事情，不用等待耗时任务执行完成，等我们干完事之后，再通过 Future 获取到耗时任务的执行结果
>
>- Future 是异步思想的典型运用
...
<u>处理任务结果</u> ：
- `join()` 等待异步任务的完成，并获取结果，**如果任务未完成会阻塞当前线程**
