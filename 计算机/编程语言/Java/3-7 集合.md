...
>[!quote] 哈希值 `hashcode`
>每一个对象都有属于自己的 `hashcode` ，是用于区分不同对象的**整型变量**
>
>- `hashcode` 不是内存地址
>- 默认由 Object 类中的本地方法 `hashcode()` 生成
...
>[!quote] 哈希冲突
>哈希冲突 是不同的输入值经过 `hashcode()` 处理后产生了相同的 `hashcode`【~~哈希值~~】，<u>哈希冲突是不可避免的，因为哈希函数将无限的输入映射到有限的输出</u>
>
> ```java
...
- 【增】
	...
	- `T compute(Key, BiFunction函数式接口)` 处理指定的 Key 的值。如果这个值存在，则将 Key，和该值作为函数式接口方法；如果这个值不存在，则将 Key，和 null 值作为函数式接口的入参。函数式接口中将处理指定的 Key 的值，返回一个指定的 Key 的值的引用，~~也就是你直接修改这个引用，就会影响原来的 Map~~
