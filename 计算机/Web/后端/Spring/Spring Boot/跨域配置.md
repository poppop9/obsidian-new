...
- `CorsConfigurationSource` Bean 配置：适合需要细粒度控制和与 Spring Security 配置结合的场景。
- `@CrossOrigin` 注解：适合简单场景，单个控制器或方法需要跨域访问时使用。
- 全局配置 CORS：适合全局统一的CORS配置需求，最为简洁。

对于你的情况，建议使用 `CorsConfigurationSource` 配置，因为你已经在使用 Spring Security，这种方式更易于集成和管理复杂的安全需求
