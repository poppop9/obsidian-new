...
# 未整理
第三个常见的内存泄漏来源是监听器和其他回调。如果你实现了一个 API，客户端注册了回调，但是没有显式地注销，除非你采取一些措施，否则这些回调对象就会不断累积起来。确保回调及时被垃圾收集处理的一个方法是只存储对它们的弱引用，例如，将其仅作为WeakHashMap中的键来存储。
